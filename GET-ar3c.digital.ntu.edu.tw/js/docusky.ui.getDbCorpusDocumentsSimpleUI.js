...
var ClsDocuskyGetDbCorpusDocumentsSimpleUI = function(param) {     // class (constructor)
   ...
   var init = function() {
      const forceHttps = true;                                         // 2021-12-20: 若放到 server 上，可能需設為 true（避免 BigIP 將 http 轉為 https 時沒有帶上 CORS 所需的 headers）
      ...
      if (forceHttps && scheme == 'http') scheme = 'https';             // 2021-12-20
   ...
   me.login = function(username, password, succFunc, failFunc) {        // 2021-06-13: 改 public, 加上 succFunc
      ...
      $.post(me.urlLogin, postdata, function(jsonObj) {
         ...
         if (jsonObj.code == 0) {               // successfully login
            ...
            SessionKey = jsonObj.message;                // 2021-02-06: getQueryResultDocuments() 會用到（放入 url parameter）
         }
   ...
   me.logout = function(succFunc) {                                             // 2021-06-14
      ...
      $.get(url, function(jsonObj) {
         ...
         me.hideWidget(true, false);       // 2021-12-30: 隱藏 widget 但不設定 displayWidget 變數
      }, 'json')
      .fail(function (jqXHR, textStatus, errorThrown){
        if (jqXHR.status=="200") {          // 2019-05-07: server return not correct json
           alert("Server response seems not a valid JSON");
   ...
   me.hideWidget = function(bool = true, setVar = true) {        // 2021-12-30: 加入 setVar，登出時雖然需隱藏 widget，但不應設定 displayWidget 為 false
   ...
   me.getQueryResultDocuments = function(param, evt, succFunc, failFunc) {
      ...
      // 使用本地数据文件替代远程API调用
      var localDataFiles = [
         'data/getQueryResultDocumentsJson.php',
         'data/getQueryResultDocumentsJson_2.php',
         'data/getQueryResultDocumentsJson_3.php',
         'data/getQueryResultDocumentsJson_4.php',
         'data/getQueryResultDocumentsJson_5.php'
      ];

      var allDocList = [];
      var loadedCount = 0;
      var totalFiles = localDataFiles.length;
      ...
      // 加载所有本地数据文件
      localDataFiles.forEach(function(fileUrl) {
         $.getJSON(fileUrl, function(data) {
            if (data.code == 0 && data.message.docList) {
               allDocList = allDocList.concat(data.message.docList);
            }
            loadedCount++;
            ...
            if (loadedCount >= totalFiles) {
               ...
               var mergedData = {
                  code: 0,
                  message: {
                     totalFound: allDocList.length,
                     page: 1,
                     pageSize: allDocList.length,
                     docList: allDocList,
                     target: 'OPEN',
                     db: '春秋三傳',
                     corpus: '[ALL]'
               ...
               // 更新对象状态
               me.totalFound = mergedData.message.totalFound;
               me.page = mergedData.message.page;
               me.pageSize = mergedData.message.pageSize;
               me.docList = mergedData.message.docList.slice();
               me.target = mergedData.message.target;
               me.db = mergedData.message.db;
               me.corpus = mergedData.message.corpus;
               
               if (typeof me.callerCallback === "function") {
                  me.callerCallback(mergedData);
               }
            }
         }).fail(function() {
            loadedCount++;
            if (loadedCount >= totalFiles && allDocList.length > 0) {
               $("#" + loadingContainerId).hide();
               var mergedData = {
                  code: 0,
                  message: {
            ...
            } else if (loadedCount >= totalFiles) {
               $("#" + loadingContainerId).hide();
               alert("无法加载本地数据文件，请检查 data 目录是否存在。");
            }
         });
      });
   };
